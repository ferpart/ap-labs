...
For the laboratory 4.2 we were assigned to do matrix multiplication with thw following instructions.
...
To compile the code the user needs to do the following:
...
To run the program the user needs to do the following:

```bash

#NUMBER is any number bigger or equal to 8
./multiplier -n [NUMBER]
```

#### All of the results will be shown on the "Results.dat" file.
