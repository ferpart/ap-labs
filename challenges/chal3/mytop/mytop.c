...
void clear(),
     ...
     filePrinter(int fd),
     interruptionWriter(int signal),
     forceExit(int signal),
     cleanup(void);
...
void topWriter(char *tempFile){
	...
	sprintf(result , "____________________________________________________________________________________________________________________________________\n");
	write(fd, result, strlen(result));
	//result[0]='\0';
 	sprintf(result, "|  %-9s|  %-9s|  %-41s|  %-15s|  %-15s|  %-9s|  %-12s|\n", "PID", "Parent", "Name", "State", "Memory", "#Threads", "Open Files");
	write(fd, result, strlen(result));
...
char *statusExplorer(char *path, int openFiles){
	...
	sprintf(returnValue, "|  %-9s|  %-9s|  %-41s|  %-15s|  %-15s|  %-9s|  %-12d|\n", pid, ppid, name, state, memory, threads, openFiles);
